...
On DEVRAIT utiliser le null coalescing operator (`??`) plutôt que l'OR operator (`||`) pour définir une valeur par défaut SAUF dans le cas où on veux explicitement remplacer les chaines vide, les `0` et `false`.
...
```tsx
const value = props.value || 'default value';
```
...
L'OR operator (`||`) retourne la valeur de droite si la valeur de gauche est "falsy" (c'est à dire `false`, `0`, `''`, `null`, `undefined` ou `NaN`).

Le null coalescing operator (`??`) retourne la valeur de droite si la valeur de gauche est `null` ou `undefined`.

Cela permet d'éviter des bugs lorsque la valeur de gauche peut être `0` ou `''` (ex: un compteur, un champ texte...) et que l'on souhaite conserver cette valeur.
:::

:::note En savoir plus

- [Documentation MDN sur le null coalescing operator (??)](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Nullish_coalescing)
- [Documentation MDN sur l'OR operator (||)](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Logical_OR)

:::

## Types vs interface

On DEVRAIT utiliser des `type` plutôt que des `interface` pour définir des types d'objets.
...
[Documentation officielle "Differences Between Type Aliases and Interfaces".](https://www.typescriptlang.org/docs/handbook/2/everyday-types.html#differences-between-type-aliases-and-interfaces)
:::

## Array&lt;T&gt; vs T[]

On DOIT utiliser la syntaxe `Array<T>` pour définir un tableau d'éléments de type `T`.
...
La syntaxe `Array<T>` est plus explicite et permet de mieux comprendre le type des éléments du tableau. De plus, elle est plus cohérente avec d'autres types génériques en TypeScript.

Array est juste un générique comme un autre, exactement comme `List<T>` par exemple (ou n'importe quelle structure y compris dans le code JS comme `Set<T>`).

En savoir plus : [Array Types in TypeScript](https://tkdodo.eu/blog/array-types-in-type-script);
